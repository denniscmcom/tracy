...
impl Mat for Metal {
    fn scatter(&self, ray: Ray, hit: Hit) -> Option<ScatterData> {
        ...
        dir = dir_u + (self.fuzz * random_vec_u());
...
fn reflect(v: &Vec3D<f64>, norm: &Vec3D<f64>) -> Vec3D<f64> {
...
fn refract(
    ...
    let r_out_perp = etai_over_etat * (*v_u + cos_theta * *norm);
    let r_out_parallel = -f64::sqrt(f64::abs(1.0 - r_out_perp.len_2())) * *norm;
